...
copy_buffer_to_image :: proc(g: ^Graphic, buffer: vk.Buffer, image: vk.Image, width: u32, height: u32) {
	command_buffer := begin_single_command(g)
...
create_image :: proc(
	...
	must(vk.CreateImage(g.device, &image_info, nil, &image))

	// Allocating memory
	memory_requirements: vk.MemoryRequirements
	vk.GetImageMemoryRequirements(g.device, image, &memory_requirements)

	memory_type, ok := find_memory_type(g.physical_device, memory_requirements.memoryTypeBits, memory_properties)
	...
	must(vk.AllocateMemory(g.device, &allocate_info, nil, &image_memory))

	vk.BindImageMemory(g.device, image, image_memory, 0)
...
generate_mipmaps :: proc(
	...
	for i in 1 ..< mip_levels {
		...
		vk.CmdPipelineBarrier(cb, {.TRANSFER}, {.TRANSFER}, {}, 0, nil, 0, nil, 1, &barrier)
		...
		vk.CmdBlitImage(cb, image, .TRANSFER_SRC_OPTIMAL, image, .TRANSFER_DST_OPTIMAL, 1, &blit, .LINEAR)
		...
		vk.CmdPipelineBarrier(cb, {.TRANSFER}, {.FRAGMENT_SHADER}, {}, 0, nil, 0, nil, 1, &barrier)
	...
	end_single_command(g, cb)
}

create_texture_image :: proc(g: ^Graphic, image: Image, mip_levels: u32 = 1) -> TextureImage {
	...
	// Create Vulkan Image
	vk_image, vk_memory := create_image(
		g,
		image.width,
		image.height,
		mip_levels,
	...
	transition_image_layout(g, vk_image, {.COLOR}, format, .UNDEFINED, .TRANSFER_DST_OPTIMAL, mip_levels)
...
destroy_texture_image :: proc(g: ^Graphic, image: ^TextureImage) {
	vk.DestroyImageView(g.device, image.view, nil)
	vk.DestroyImage(g.device, image.image, nil)
	vk.FreeMemory(g.device, image.memory, nil)
	image.view = 0
	image.image = 0
	image.memory = 0
}

create_texture :: proc(g: ^Graphic, image: Image, mip_levels: f32 = 0) -> Texture {
	levels: f32 = 0
	if mip_levels <= 0 {
...
destroy_texture :: proc(g: ^Graphic, texture: ^Texture) {
	vk.DestroySampler(g.device, texture.sampler, nil)
	texture.sampler = 0
	destroy_texture_image(g, &texture.image)
}
